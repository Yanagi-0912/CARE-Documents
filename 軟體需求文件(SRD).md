...
| FR-3.1   | 一般醫療資源搜尋  | 使用者輸入要查詢的醫療院所名稱，由系統回覆院所資訊   |
| FR-4   | 實體協助功能    | 提供使用者如何取得資源及掛號資訊        |
| FR-4.1   | 協助使用者撥打電話    | 提供使用者按鈕，點擊後跳轉至手機內建撥打電話App        |
| FR-5   | 健康問題諮詢    | 若為一般健康問題，系統需進入文字內容驗證流程                |
| FR-5.1   | 健康問題諮詢紀錄與摘要    | 紀錄使用者的訊息，並定期產生摘要，方便使用者檢視               |
...
| FR-8   | TTS 語音輸出  | 可在richmenu選擇是否將最終回應內容以 TTS（Text-to-Speech）方式讀出      |
| FR-9   | 多語言支援     | TTS 支援國語/台語（預設台語），文字支援國語及外語(英、日、菲、泰、越南、印尼語)           |
| FR-10  | 家庭連結功能    | 家庭成員可互相查看健康狀態與資訊並進行管理                 |
| FR-10.1  | 家庭連結功能-用藥提醒    | 長輩點擊了 LINE 上的「已服藥」按鈕，家人端就可以看到標記  |
| FR-10.2  | 家庭連結功能-家庭群組監護人(guardian)    | 監護人擁有管理一般成員的所有資訊，包含:聊天紀錄、健康資料  |
| FR-10.3  | 家庭連結功能-家庭群組照護者(caregiver)    | 擁有讀權限。除了看資料，還能「代為輸入」資料  |
| FR-10.4  | 家庭連結功能-家庭群組一般成員(member)    | 擁有自己的資料，並可透過設定決定要分享哪些數據給家庭。  |
...
| NFR-5      | 可用性         | 一般操作與 LINE 訊息應於合理時間內完成回應，避免使用者等待過久  |
| NFR-6      | 資料安全性    | 個案、居服員及服務紀錄等敏感資料須進行適當的存取控制與傳輸保護    |
| NFR-7      | 可維護性    | 系統各功能模組應具備良好區隔，便於後續功能擴充與維護         |
